...
// Get recent workouts
$stmt = $db->prepare("SELECT w.*, COUNT(ws.id) as set_count FROM workouts w LEFT JOIN workout_sets ws ON w.id = ws.workout_id WHERE w.user_id = ? AND w.ended_at IS NOT NULL GROUP BY w.id ORDER BY w.started_at DESC LIMIT 5");
$stmt->execute([$_SESSION['user_id']]);
$recentWorkouts = $stmt->fetchAll();

// Helper to get exercises for a workout
...
function getMuscleGroups($exercises) {
    ...
    foreach ($exercises as $name) {
        ...
        if (str_contains($nameLower, 'press') || str_contains($nameLower, 'fly') || str_contains($nameLower, 'chest') || str_contains($nameLower, 'pec')) {
            ...
            $groups[] = 'shoulders';
        } elseif (str_contains($nameLower, 'curl') || str_contains($nameLower, 'bicep') || str_contains($nameLower, 'tricep') || str_contains($nameLower, 'extension')) {
            $groups[] = 'arms';
        }
    }
